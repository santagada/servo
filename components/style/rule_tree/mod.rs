...
impl RuleTree {
    ...
    pub fn insert_ordered_rules<'a, I>(&self, iter: I) -> StrongRuleNode
        where I: Iterator<Item=&'a Arc<Rule>>
    {
        let root = self.root.clone();
        let mut current = self.root.clone();
        for rule in iter {
            current = current.ensure_child(&root, rule);
        }
        current
    }
...
struct RuleNode {
    ...
    /// The actual rule node. Only the root has no rule node.
    ...
    fn dump<W: Write>(&self, writer: &mut W, indent: usize) {
        ...
        match self.rule {
            Some(ref rule) => {
                ...
                let _ = write!(writer, "{:?}", rule.selector);
            }
            None => {
                if indent != 0 {
...
impl StrongRuleNode {
    ...
    fn ensure_child(&self, root: &StrongRuleNode, rule: &Arc<Rule>) -> StrongRuleNode {
        let mut last = None;
        let mut iter = self.get().children.iter();
        while let Some(current) = iter.next() {
            if arc_ptr_eq(current.get().rule.as_ref().unwrap(), rule) {
                return current;
            }
            last = Some(current);
        }
